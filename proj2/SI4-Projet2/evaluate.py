...
def runDataset(name, i) :
    ...
    try:
        ...
        if not isfile(binFile) :
            return "Error"
        output = subprocess.check_output(["mpirun", "-np", "4", binFolder+name, dataFolder+"a_"+str(i), dataFolder+"v_"+str(i)], stderr=STDOUT, timeout=5,universal_newlines=True).strip().replace('\n', '').replace(' ', '')
        # print(output)
